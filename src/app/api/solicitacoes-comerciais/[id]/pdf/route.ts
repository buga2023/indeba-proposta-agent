...
import { fichaDaSolicitacao } from "@/lib/ferramentas-comerciais";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const ficha = await fichaDaSolicitacao(usuario, id);
    if (!ficha) return NextResponse.json({ erro: "Solicitação não encontrado(a)." }, { status: 404 });
    const pdf = await pdfDaFicha(ficha, dataUri("/marca/indeba-logo.png"));
    return new NextResponse(new Uint8Array(pdf), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `inline; filename="${nomeArquivo("solicitacao", ficha.cliente)}"`,
